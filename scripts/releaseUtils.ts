...
console.log("args======", pico.cyan(pico.yellow(JSON.stringify(args))))
if (isDryRun) {
...
export async function run(bin: string, args: string[], opts: ExecaOptions<string> = {}) {
    console.log(pico.cyan(`> Running: ${bin} ${args.join(" ")} in ${opts.cwd || process.cwd()}`))
...
export async function publishPkg(pkdDir: string, tag?: string): Promise<void> {
  ...
  console.log(pico.cyan(`run publishPkg info: ${JSON.stringify(publicArgs)}`))
